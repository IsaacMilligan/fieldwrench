...
async function wipeDemo(sql: Sql) {
  ...
  await sql`DELETE FROM job_template_lines WHERE template_id IN (SELECT id FROM job_templates WHERE shop_id = ${DEMO_SHOP_ID})`;
  await sql`DELETE FROM job_templates WHERE shop_id = ${DEMO_SHOP_ID}`;
