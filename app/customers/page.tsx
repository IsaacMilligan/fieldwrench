        ...
        {rows.map((c) => (
          <li key={String(c.id)}>
            <Link href={`/customers/${c.id}`} className="panel block">
              ...
              <div className="text-sm text-muted">{formatPhone(c.phone)}</div>
              <div className="text-sm text-steel">{vehicleNoun(Number(c.vehicle_count))}</div>
              {c.last_visit ? (
                <div className="text-sm text-muted">Last visit {formatDate(c.last_visit as string)}</div>
              ) : null}
            </Link>
          </li>
        ))}
